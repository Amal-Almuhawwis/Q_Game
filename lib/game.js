...
const createWaitingGame = async (gameName, gameId) => {
  const result = await WaitingGamesColl.insertOne({gameName, gameId});
  return result.insertedId != null;
};

const create = async (gameName, playerId) => {
  ...
  if (!(await createWaitingGame(gameName, gameId.toHexString()))) {
    return {
...
const deleteGame = async (gameId) => {
  const res = await GamesColl.deleteOne({_id: new ObjectId(gameId)});
...
const leave = async (gameId, winnerID) => {
  const looserID = 'p1' === winnerID ? 'p2' : 'p1';
  const g = await find(gameId);
  if (!g) {
    return {
      error: 'Game does not exists'
    };
  ...
  // handle if user leave the game while waiting for an opponent
  if (await isWaitingGame(gameId)) {
    await User.setCurrentlyPlaying(g.private.players.playerOne, 0);
    await deleteWaitingGame(gameId);
    await deleteGame(gameId);
    return {
      io: gameId,
      public: {message: 'Game ended'}
    };
  }

  const looser_name = g.public.gameState.playerName[looserID];
  const winner_name = g.public.gameState.playerName[winnerID];
  g.public.message = `game is finished, ${looser_name} leave the game, and ${winner_name} won.`;

  g.public.gameState.winner = g.public.gameState.playerName[winnerID];
  await User.setCurrentlyPlaying(g.private.players.playerOne, 0);
  await User.setCurrentlyPlaying(g.private.players.playerTwo, 0);

  g.public.gameState.history.push({
    action: 'message',
    message: g.public.message
  });

  const res = await GamesColl.replaceOne({_id: new ObjectId(gameId)}, g);
  if (res.modifiedCount !== 1) {
    return {
...
const getAll = async () => {
  ...
  for (let g, i = 0; i < games.length; i++) {
    ...
    all.push({
      ...
      playerOne: g.public.gameState.playerName.p1,
      playerTwo: g.public.gameState.playerName.p2,
      playerOneIsWinner: g.public.gameState.playerName.p1 === g.public.gameState.winner,
      playerTwoIsWinner: g.public.gameState.playerName.p2 === g.public.gameState.winner
    });
  }
  return all;
};
...
module.exports = {
  find, count, create, join, getAll, leave, getWaitingList, makeMove, makeWall
};