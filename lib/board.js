class Board {
  constructor(game) {
    ...
    this.board[game.public.gameState.pawns.p1.x][game.public.gameState.pawns.p1.y].pawn = 'p1';
    this.p1 = {
      x: game.public.gameState.pawns.p1.x,
      y: game.public.gameState.pawns.p1.y
    }
  
    this.board[game.public.gameState.pawns.p2.x][game.public.gameState.pawns.p2.y].pawn = 'p2';
    this.p2 = {
      x: game.public.gameState.pawns.p2.x,
      y: game.public.gameState.pawns.p2.y
    }
  ...
  shortestPath(playerId) {
    ...
    while (Q.length > 0) {
      ...
      // prioritize move left first
      if (0 === destX) {
        // visit left
        if ((node.x - 1) >= 0 && !node.left) {
          if ((n = this._visit(node, _board[node.x - 1][node.y]))) {
            Q.push(n);
          }
        }
      }
      // prioritize move right first
      else {
        // visit right
      ...
      if ((node.y - 1) >= 0 && !node.top) {
        if ((n = this._visit(node, _board[node.x][node.y - 1]))) {
          Q.push(n);
        }
      }
