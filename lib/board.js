  ...
  shortestPath(playerId) {
    ...
    while (Q.length > 0) {
      ...
      if ((node.y - 1) >= 0 && !node.top) {
        if (_board[node.x][node.y - 1].pawn) {
          if ((n = this._visit(node, _board[node.x][node.y - 2]))) {
            Q.push(n);
          }
        }
        else if ((n = this._visit(node, _board[node.x][node.y - 1]))) {
          Q.push(n);
        }
      }
      ...
      if ((node.x + 1) < 9 && !node.right) {
        if (_board[node.x + 1][node.y].pawn) {
          if ((n = this._visit(node, _board[node.x + 2][node.y]))) {
            Q.push(n);
          }
        }
        else if ((n = this._visit(node, _board[node.x + 1][node.y]))) {
          Q.push(n);
        }
      }
